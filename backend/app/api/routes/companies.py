...
async def get_company(ticker: str, db: Session = Depends(get_db)):
    ...
    fundamental_data = await yfinance_provider.build_fundamental_data(ticker.upper())
...
async def get_fundamentals(ticker: str):
    ...
    """
    income = await yfinance_provider.get_income_statement(ticker.upper(), "annual", 5)
    balance = await yfinance_provider.get_balance_sheet(ticker.upper(), "annual", 5)
    cashflow = await yfinance_provider.get_cash_flow_statement(ticker.upper(), "annual", 5)
...
async def get_prices(
    ...
    prices = await yfinance_provider.get_historical_prices(ticker.upper(), from_date, to_date)
