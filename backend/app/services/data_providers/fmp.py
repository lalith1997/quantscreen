...
class FMPProvider:
    ...
    async def get_company_profile(self, ticker: str) -> Optional[dict]:
        ...
        """
        try:
            response = await self.client.get(self._url(f"profile?symbol={ticker}"))
    ...
    async def get_income_statement(self, ticker: str, period: str = "annual", limit: int = 5) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"income-statement?symbol={ticker}&period={period}&limit={limit}")
            )
            response.raise_for_status()
    ...
    async def get_balance_sheet(self, ticker: str, period: str = "annual", limit: int = 5) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"balance-sheet-statement?symbol={ticker}&period={period}&limit={limit}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_cash_flow_statement(self, ticker: str, period: str = "annual", limit: int = 5) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"cash-flow-statement?symbol={ticker}&period={period}&limit={limit}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_key_metrics(self, ticker: str, period: str = "annual", limit: int = 5) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"key-metrics?symbol={ticker}&period={period}&limit={limit}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_financial_ratios(self, ticker: str, period: str = "annual", limit: int = 5) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"ratios?symbol={ticker}&period={period}&limit={limit}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_quote(self, ticker: str) -> Optional[dict]:
        ...
        try:
            response = await self.client.get(self._url(f"quote?symbol={ticker}"))
            response.raise_for_status()
            data = response.json()
            if data and len(data) > 0:
                return data[0]
    ...
    async def get_historical_prices(
        ...
        try:
            url = f"historical-price-eod?symbol={ticker}"
            if from_date:
            ...
            response.raise_for_status()
            data = response.json()
            # Stable endpoint returns list directly (not nested under "historical")
    ...
    async def get_sp500_constituents(self) -> list[dict]:
        ...
        try:
            response = await self.client.get(self._url("sp500-constituent"))
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_earnings_calendar(
        ...
        try:
            response = await self.client.get(
                self._url(
                    f"earning-calendar?from={from_date.isoformat()}&to={to_date.isoformat()}"
                )
            )
            response.raise_for_status()
            return response.json()
    ...
    async def get_earnings_surprises(self, ticker: str) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"earnings-surprises?symbol={ticker}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_stock_news(self, ticker: str, limit: int = 50) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"news?tickers={ticker}&limit={limit}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_general_news(self, page: int = 0, size: int = 20) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"news?limit={size}&page={page}")
            )
            response.raise_for_status()
            data = response.json()
    ...
    async def get_press_releases(self, ticker: str, limit: int = 10) -> list[dict]:
        ...
        try:
            response = await self.client.get(
                self._url(f"press-releases?symbol={ticker}&limit={limit}")
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_market_indexes(self) -> list[dict]:
        ...
        results = []
        try:
            sp500 = await self.client.get(self._url("quote?symbol=%5EGSPC"))
            if sp500.status_code == 200:
                results.extend(sp500.json())
        except Exception as e:
            print(f"Error fetching S&P 500 index: {e}")
        try:
            vix = await self.client.get(self._url("quote?symbol=%5EVIX"))
    ...
    async def get_gainers_losers(self) -> dict:
        ...
        try:
            gainers = await self.client.get(self._url("gainers"))
            if gainers.status_code == 200:
                result["gainers"] = gainers.json()
        except Exception as e:
            print(f"Error fetching gainers: {e}")
        try:
            losers = await self.client.get(self._url("losers"))
